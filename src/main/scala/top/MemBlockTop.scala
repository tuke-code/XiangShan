...
import xiangshan.frontend.FrontendDebugTopDownInfo
...
class MemBlockTop()(implicit p: Parameters) extends LazyModule
  ...
  private def makeClientNode(name: String) = {
    TLClientNode(Seq(TLMasterPortParameters.v1(
      Seq(TLMasterParameters.v1(name, IdRange(0, 1)))
    )))(ValName(name))
  }

  private val frontendICacheParams = coreParams.frontendParameters.icacheParameters
  ...
  private val frontendICacheClientNode = TLClientNode(Seq(TLMasterPortParameters.v1(
    Seq(TLMasterParameters.v1(
      ...
      sourceId = IdRange(0, frontendICacheParams.NumFetchMshr + frontendICacheParams.NumPrefetchMshr + 1)
    )),
    requestFields = Seq(
      ReqSourceField(),
      MemBackTypeMMField()
  ...
  private val frontendICacheCtrlFrontendNode = Option.when(icacheCtrlEnabled) {
    TLManagerNode(Seq(TLSlavePortParameters.v1(
      managers = Seq(TLSlaveParameters.v1(
        address = Seq(icacheCtrlAddress),
        regionType = RegionType.UNCACHED,
        executable = false,
        supportsGet = TransferSizes(1, frontendICacheParams.ctrlUnitParameters.BeatBytes),
        supportsPutPartial = TransferSizes(1, frontendICacheParams.ctrlUnitParameters.BeatBytes),
        supportsPutFull = TransferSizes(1, frontendICacheParams.ctrlUnitParameters.BeatBytes),
        fifoId = Some(0)
      )),
      beatBytes = frontendICacheParams.ctrlUnitParameters.BeatBytes
  ...
  val frontendICacheCtrlIONode = Option.when(icacheCtrlEnabled) {
    TLClientNode(Seq(TLMasterPortParameters.v1(
  ...
  private val uncacheExternalAddress = {
    ...
    val internalMmio = dcacheParameters.cacheCtrlAddressOpt.toSeq
  ...
  frontendICacheCtrlFrontendNode.foreach(_ := memBlock.inner.frontendBridge.icachectrl_node)
  ...
  frontendICacheCtrlIONode.foreach(node => memBlock.inner.frontendBridge.icachectrl_node := node)
  ...
  val frontendICacheCtrlMemPort = frontendICacheCtrlIONode.map(node => InModuleBody(node.makeIOs()))
  ...
  val frontendICacheCtrlFrontendPort = frontendICacheCtrlFrontendNode.map(node => InModuleBody(node.makeIOs()))
...
class MemBlockTopImp(wrapper: MemBlockTop) extends LazyRawModuleImp(wrapper) {
  ...
  // Preserve frontend-related CSR leaf ports on the generated MemBlock module
  // so its port list stays aligned with the SoC-generated variant.
  ...
  dontTouch(wrapper.memBlock.module.io.ooo_to_mem.csrCtrl.bp_ctrl.ubtbEnable)
  dontTouch(wrapper.memBlock.module.io.ooo_to_mem.csrCtrl.bp_ctrl.abtbEnable)
  dontTouch(wrapper.memBlock.module.io.ooo_to_mem.csrCtrl.bp_ctrl.mbtbEnable)
  dontTouch(wrapper.memBlock.module.io.ooo_to_mem.csrCtrl.bp_ctrl.tageEnable)
  dontTouch(wrapper.memBlock.module.io.ooo_to_mem.csrCtrl.bp_ctrl.scEnable)
  dontTouch(wrapper.memBlock.module.io.ooo_to_mem.csrCtrl.bp_ctrl.ittageEnable)
...
object MemBlockTopMain extends App {
  ...
  // Standalone MemBlock does not instantiate Backend, so bind the mutable
  // backend/issue/exu parameter cross-links before any MemBlock-facing bundle
  // types are materialized in this wrapper.
  ...
  for (schdCfg <- backendParams.allSchdParams) {
    ...
    for (iqCfg <- schdCfg.issueBlockParams) {
      ...
      iqCfg.allExuParams.foreach(_.bindIssueBlockParam(iqCfg))
    }
  }

  private val unboundIssueExus = backendParams.allSchdParams.flatMap(_.issueBlockParams).flatMap { iqCfg =>
    iqCfg.allExuParams.collect {
      case exuCfg if exuCfg.issueBlockParam == null => s"${iqCfg.schdType}:${iqCfg.idxInSchBlk}:${exuCfg.name}"
    }
  }
  require(unboundIssueExus.isEmpty, s"Unbound issueBlockParam for exus: ${unboundIssueExus.mkString(", ")}")

  val soc = DisableMonitors(p => LazyModule(new MemBlockTop()(p)))(memBlockConfig)
  Generator.execute(firrtlOpts, soc.module, firtoolOpts)

  FileRegisters.write(fileDir = "./build-memblock", filePrefix = "MemBlockTop.")
}
