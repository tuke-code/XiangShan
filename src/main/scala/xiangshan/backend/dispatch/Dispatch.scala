...
class Dispatch(implicit p: Parameters) extends XSModule with HasPerfEvents with HasVLSUParameters {
  ...
  needMultiExu.zipWithIndex.map{ case ((fus, exuidx), needMultiExuidx) => {
    ...
    val IQSortReadyNum = VecInit((0 until iqNum + 1).map{ i =>
      val takeNum = if (i == 0) 1 else i
      val takeReadyNum = VecInit(IQSort.take(takeNum))
...
//  })
  ...
  issueQueueSizeVec := allIssueParams.map{ param =>
    println(s"dispatch: issuequeuenum $issueQueueNum")
    println(s"allissueparams size: ${allIssueParams.size}")
    println(s"Dispatch: issuequeue entries: ${param.numEntries}")
