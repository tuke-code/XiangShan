...
object EntryBundles extends HasCircularQueuePtrHelper {
  ...
  class Status(implicit p: Parameters, params: IssueBlockParams) extends XSBundle {
    ...
    val earlyFmaSrc2          = Option.when(params.needEarlyFmaWakeup)(Bool())
    val earlyFmaSource        = Option.when(params.needEarlyFmaWakeup)(UInt(backendParams.numExu.W))
    val earlyFmaSrc2UseBypass = Option.when(params.needEarlyFmaWakeup)(Bool())
    //src status
    ...
    def srcReady: Bool        = {
      VecInit(srcStatus.map(_.srcState).map(SrcState.isReady)).asUInt.andR &&
        ...
        srcStatusV0.map(_.srcState).map(SrcState.isReady).getOrElse(true.B)
    }
  ...
  class V0SrcStatus(implicit p: Parameters, params: IssueBlockParams) extends XSBundle {
  ...
  class EntryBundle(isDeq: Boolean = false)(implicit p: Parameters, params: IssueBlockParams) extends XSBundle {
    ...
    val v0Ren                 = Option.when(isDeq && params.readV0Rf )(Bool())
    ...
    def genXrfRen(entry: EntryBundle): Unit = {
      ...
      this.v0Ren.foreach(x => x := entry.status.srcStatusV0.get.dataSource.readReg)
    }
  }
  ...
  def CommonWireConnect(common: CommonWireBundle, hasIQWakeup: Option[CommonIQWakeupBundle], validReg: Bool, og1Payload: EntryOg1Payload, status: Status, commonIn: CommonInBundle, isEnq: Boolean)(implicit p: Parameters, params: IssueBlockParams) = {
    ...
    val earlyFmaWait          = params.needEarlyFmaWakeup.B &&
                                  status.isFmac.getOrElse(false.B) &&
                                  status.earlyFmaSrc2.getOrElse(false.B) &&
                                  status.issueTimer =/= 3.U
    val earlyFmaComplete      = params.needEarlyFmaWakeup.B &&
                                  status.isFmac.getOrElse(false.B) &&
                                  status.earlyFmaSrc2.getOrElse(false.B) &&
                                  status.issueTimer === 3.U
    common.deqSuccess         := status.issued && ((finalSuccess && !earlyFmaWait) || earlyFmaComplete) && !common.srcLoadCancelVec.asUInt.orR
    ...
    if (params.readVlRf) {
      commonIn.wakeUpFromWB.foreach(
        x => println(s"[$params] tmp: ${x.bits.exuIndices} ${x.bits.dataConfig}")
      )
    ...
    if (params.readV0Rf) {
      val wakeUpFromV0 = Cat(commonIn.wakeUpFromWB.filter(_.bits.dataConfig.isInstanceOf[V0Data]).map{ bundle =>
        bundle.bits.wakeUpV0((status.srcStatusV0.get.psrc, SrcType.v0), bundle.valid)
      }).orR
      common.srcV0WakeupByWB := wakeUpFromV0
    } else {
      common.srcV0WakeupByWB := false.B
    }
  }
  ...
  def CommonIQWakeupConnect(common: CommonWireBundle, hasIQWakeupGet: CommonIQWakeupBundle, validReg: Bool, status: Status, commonIn: CommonInBundle, isEnq: Boolean)(implicit p: Parameters, params: IssueBlockParams) = {
    val wakeupVec: Seq[Seq[Bool]] = commonIn.wakeUpFromIQ.map{(bundle: ValidIO[IssueQueueIQWakeUpBundle]) =>
      ...
      if (params.readVecRf) {
        ...
        bundle.bits.wakeUpV0FromIQ(status.srcStatusV0.get.psrc,SrcType.v0) :+
    ...
    if (params.needEarlyFmaWakeup) {
      ...
      val earlyFmaWakeupVec = commonIn.earlyFmaWakeup.get.map { wakeup =>
        wakeup.valid &&
          wakeup.bits.fpWen &&
          wakeup.bits.pdest === src2Status.psrc &&
          SrcType.isFp(src2Status.srcType) &&
          SrcState.isBusy(src2Status.srcState) &&
          status.isFmac.getOrElse(false.B) &&
          !normalSrc2Wakeup
      }
      hasIQWakeupGet.earlyFmaSrc2Wakeup := VecInit(earlyFmaWakeupVec).asUInt.orR
      hasIQWakeupGet.earlyFmaSrc2SourceOH := Mux1H(
        earlyFmaWakeupVec,
        commonIn.earlyFmaWakeup.get.map(w => MathUtils.IntToOH(w.bits.exuIdx).U(p(XSCoreParamsKey).backendParams.numExu.W))
      )
    } else {
      hasIQWakeupGet.earlyFmaSrc2Wakeup := false.B
      hasIQWakeupGet.earlyFmaSrc2SourceOH := 0.U
    }
  ...
  def EntryRegCommonConnect(common: CommonWireBundle, hasIQWakeup: Option[CommonIQWakeupBundle], validReg: Bool, entryUpdate: EntryBundle, entryReg: EntryBundle, status: Status, commonIn: CommonInBundle, isEnq: Boolean, isComp: Boolean)(implicit p: Parameters, params: IssueBlockParams) = {
    ...
    val earlyFmaComplete                               = params.needEarlyFmaWakeup.B && status.isFmac.getOrElse(false.B) && status.earlyFmaSrc2.getOrElse(false.B) && status.issueTimer === 3.U
    ...
    val earlyFmaSrc2WakeupNow = if (params.needEarlyFmaWakeup) {
      hasIQWakeupGet.earlyFmaSrc2Wakeup &&
    ...
    entryUpdate.status.earlyFmaSrc2.foreach(_         := status.earlyFmaSrc2.get)
    entryUpdate.status.earlyFmaSource.foreach(_       := status.earlyFmaSource.get)
    entryUpdate.status.earlyFmaSrc2UseBypass.foreach(_ := status.earlyFmaSrc2UseBypass.get)
    ...
    entryUpdate.status.srcStatus.zip(status.srcStatus).zipWithIndex.foreach { case ((srcStatusNext, srcStatus), srcIdx) =>
      ...
      if(params.hasIQWakeUp) {
        ...
        srcStatusNext.srcLoadDependency               := Mux(wakeupByIQ,
                                                            ...
                                                            Mux(wakeupByEarlyFma, 0.U.asTypeOf(srcStatusNext.srcLoadDependency), common.srcLoadDependencyNext(srcIdx)))
      } else {
    ...
    entryUpdate.status.srcStatusV0.zip(status.srcStatusV0).foreach {
      case (srcStatusV0Next, srcStatusV0) =>
        val wakeupV0ByWB = common.srcV0WakeupByWB
        srcStatusV0Next.psrc     := srcStatusV0.psrc
        srcStatusV0Next.srcState := srcStatusV0.srcState | wakeupV0ByWB
        srcStatusV0Next.dataSource.value := MuxCase(
          srcStatusV0.dataSource.value,
          // no IQ wakeup here, so make it unchange since enq
          Seq(
          ),
        )
    ...
    if (params.needEarlyFmaWakeup) {
      ...
      val normalSrc2WakeupNow = hasIQWakeupGet.srcWakeupByIQ(2).asUInt.orR || common.srcWakeupByWB(2)
      val issueWithEarlyFmaSrc2 = commonIn.deqSel && !cancelBypassVec.asUInt.orR &&
      ...
      entryUpdate.status.earlyFmaSrc2.get := Mux(
        setEarlyFmaSrc2,
        true.B,
        Mux(clearEarlyFmaSrc2, false.B, status.earlyFmaSrc2.get)
      )
      entryUpdate.status.earlyFmaSource.get := Mux(
        setEarlyFmaSrc2,
        hasIQWakeupGet.earlyFmaSrc2SourceOH,
        status.earlyFmaSource.get
      )
      entryUpdate.status.earlyFmaSrc2UseBypass.get := Mux(
        issueWithEarlyFmaSrc2,
        !setEarlyFmaSrc2,
  ...
  class EnqDelayOutBundle(implicit p: Parameters, params: IssueBlockParams) extends XSBundle {
    ...
    val srcV0WakeUpByWB: Option[UInt]                       = Option.when(params.readV0Rf)(SrcState())
  ...
  def EnqDelayWakeupConnect(enqDelayIn: EnqDelayInBundle, enqDelayOut: EnqDelayOutBundle, status: Status, delay: Int)(implicit p: Parameters, params: IssueBlockParams) = {
    ...
    enqDelayOut.srcV0WakeUpByWB.foreach(
      _ := enqDelayIn.wakeUpFromWB.filter(_.bits.dataConfig.isInstanceOf[V0Data]).map {
        wakeup =>
          wakeup.bits.wakeUpV0((status.srcStatusV0.map(_.psrc).getOrElse(0.U), SrcType.v0), wakeup.valid)
      }.fold(false.B)(_ || _)
    )
    ...
    if (params.hasIQWakeUp) {
      val wakeupVec: IndexedSeq[IndexedSeq[Bool]] = enqDelayIn.wakeUpFromIQ.map{ x =>
        ...
        if (params.readVecRf) {
          ...
          x.bits.wakeUpV0FromIQ((status.srcStatusV0.get.psrc, SrcType.v0)) :+
