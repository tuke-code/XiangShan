  ...
  if (!env.FPGAPlatform) {
    ...
    for (wb <- exuWBs) {
      when(wb.valid) {
        ...
        val perfDebugInfo = wb.bits.perfDebugInfo.getOrElse(0.U.asTypeOf(new PerfDebugInfo))
        debug_instData.robIdx := idx
        debug_instData.dvaddr := wb.bits.debug.vaddr
        debug_instData.dpaddr := wb.bits.debug.paddr
        debug_instData.issueTime := perfDebugInfo.issueTime
        debug_instData.writebackTime := perfDebugInfo.writebackTime
        debug_instData.dispatchLatency := perfDebugInfo.dispatchTime - perfDebugInfo.renameTime
        debug_instData.enqRsLatency := perfDebugInfo.enqRsTime - perfDebugInfo.dispatchTime
        debug_instData.selectLatency := perfDebugInfo.selectTime - perfDebugInfo.enqRsTime
        debug_instData.issueLatency := perfDebugInfo.issueTime - perfDebugInfo.selectTime
        debug_instData.executeLatency := perfDebugInfo.writebackTime - perfDebugInfo.issueTime
        debug_instData.rsFuLatency := perfDebugInfo.writebackTime - perfDebugInfo.enqRsTime
        debug_instData.tlbLatency := perfDebugInfo.tlbRespTime - perfDebugInfo.tlbFirstReqTime
