  ...
  with HasCriticalErrors
{
  ...
  io.frontend.canAccept := decode.out.toFrontend.canAccept
  ...
  private val robIdxEqual2Redirect: Vec[Bool] = VecInit(dispatch.io.fromRename.map(_.bits.robIdx === s1_s3_redirect.bits.robIdx))
