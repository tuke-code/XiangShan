...
class StridePF()(implicit p: Parameters) extends XSModule with HasSMSModuleHelper {
  ...
  val s1_paddr_valid = RegEnable(io.s0_lookup.bits.paddr_valid, s0_valid)
  ...
  val s2_pf_gen_paddr_valid = RegEnable(s1_paddr_valid && !s1_pf_cross_page, s1_hit && s1_stride_match)
...
class ActiveGenerationTable()(implicit p: Parameters) extends XSModule with HasSMSModuleHelper {
  ...
  val s1_paddr_valid = RegEnable(s0_lookup.paddr_valid, s0_lookup_valid)
  ...
  val s1_pf_gen_paddr_valid = s1_paddr_valid && Mux(s1_pf_gen_decr_mode, !s1_vaddr_dec_cross_page, !s1_vaddr_inc_cross_page)
  ...
  s1_pht_lookup.paddr_valid := s1_paddr_valid
  ...
  val s2_pf_gen_region_addr = RegEnable(s1_pf_gen_region_addr, s1_pf_gen_valid)
  ...
  io.s2_pf_gen_req.bits.region_addr := s2_pf_gen_region_addr
...
class PatternHistoryTable()(implicit p: Parameters) extends XSModule with HasSMSModuleHelper {
  ...
  val s0_paddr_valid = lookup.valid && !evict.valid && lookup.bits.paddr_valid
  ...
  val s3_incr_paddr_valid = s3_paddr_valid && !s3_incr_crosspage
  val s3_decr_paddr_valid = s3_paddr_valid && !s3_decr_crosspage
  ...
  when(s3_cur_region_valid){
    s4_pf_gen_cur_region.region_addr := Mux(s3_paddr_valid, s3_region_paddr, s3_region_vaddr)
    ...
    s4_pf_gen_cur_region.paddr_valid := s3_paddr_valid
  ...
  when(s3_incr_region_valid){
    s4_pf_gen_incr_region.region_addr := Mux(s3_incr_paddr_valid, s3_incr_region_paddr, s3_incr_region_vaddr)
    ...
    s4_pf_gen_incr_region.paddr_valid := s3_incr_paddr_valid
  ...
  when(s3_decr_region_valid){
    s4_pf_gen_decr_region.region_addr := Mux(s3_decr_paddr_valid, s3_decr_region_paddr, s3_decr_region_vaddr)
    ...
    s4_pf_gen_decr_region.paddr_valid := s3_decr_paddr_valid
...
class SMSPrefetcher()(implicit p: Parameters) extends BasePrefecher with HasSMSModuleHelper with HasL1PrefetchSourceParameter {
  ...
  val train_paddr_valid_s0 = RegEnable(train_ld.paddr_valid, train_vld)
  ...
  active_gen_table.io.s0_lookup.bits.paddr_valid := train_paddr_valid_s0
  ...
  stride.io.s0_lookup.bits.paddr_valid := train_paddr_valid_s0
  ...
  for((train, i) <- io.ld_in.zipWithIndex){
    ...
    XSPerfAccumulate(s"pf_train_prefetched_${i}", train.valid && !train.bits.miss && isFromL1Prefetch(train.bits.metaSource))
  }
