...
//    vsMergeBuffer(i).io.fromMisalignBuffer.get.mbIndex := storeMisalignBuffer.io.toVecStoreMergeBuffer(i).mbIndex
  ...
  val mdpTrainCount = PopCount(allMdpUpdate.map(_.valid))
  val mdpUpdateWriteZero      = VecInit(allMdpUpdate.map(m => m.valid && m.bits.updateIsWriteZero))
  val mdpUpdateAllocateWeak   = VecInit(allMdpUpdate.map(m => m.valid && m.bits.updateIsAllocateWeak))
  val mdpUpdateAllocateStrong = VecInit(allMdpUpdate.map(m => m.valid && m.bits.updateIsAllocateStrong))
  val mdpUpdateNxStrong       = VecInit(allMdpUpdate.map(m => m.valid && m.bits.updateIsNxStrong))
  val mdpUpdateNxWeak         = VecInit(allMdpUpdate.map(m => m.valid && m.bits.updateIsNxWeak))
  val mdp_wz_count = PopCount(mdpUpdateWriteZero)
  val mdp_aw_count = PopCount(mdpUpdateAllocateWeak)
  val mdp_as_count = PopCount(mdpUpdateAllocateStrong)
  val mdp_is_count = PopCount(mdpUpdateNxStrong)
  val mdp_iw_count = PopCount(mdpUpdateNxWeak)
  dontTouch(mdpUpdateWriteZero)
  dontTouch(mdpUpdateAllocateWeak)
  dontTouch(mdpUpdateAllocateStrong)
  dontTouch(mdpUpdateNxStrong)
  dontTouch(mdpUpdateNxWeak)
  XSPerfAccumulate("mdp_train_count", mdpTrainCount)
  XSPerfAccumulate("mdp_wz_count", mdp_wz_count)
  XSPerfAccumulate("mdp_aw_count", mdp_aw_count)
