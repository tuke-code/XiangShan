...
//    vsMergeBuffer(i).io.fromMisalignBuffer.get.mbIndex := storeMisalignBuffer.io.toVecStoreMergeBuffer(i).mbIndex
  ...
  val mdp_wz_count = PopCount(allMdpUpdate.map(m => m.valid && m.bits.updateIsWriteZero))
  val mdp_aw_count = PopCount(allMdpUpdate.map(m => m.valid && m.bits.updateIsAllocateWeak))
  val mdp_as_count = PopCount(allMdpUpdate.map(m => m.valid && m.bits.updateIsAllocateStrong))
  val mdp_is_count = PopCount(allMdpUpdate.map(m => m.valid && m.bits.updateIsNxStrong))
  val mdp_iw_count = PopCount(allMdpUpdate.map(m => m.valid && m.bits.updateIsNxWeak))
