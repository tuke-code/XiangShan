  ...
  for (i <- 0 until LduCnt) {
    ...
    if (i == 0) {
      ...
      when (vSegmentFlag) {
        ...
        dcache.io.lsu.load(i).s2_mshr_or_tld_full_fwd := vSegmentUnit.io.rdcache.s2_mshr_or_tld_full_fwd
        ...
        dcache.io.lsu.load(i).s2_mshr_or_tld_full_fwd := newLoadUnits(i).io.dcache.s2_mshr_or_tld_full_fwd
