...
class PhrDiff(implicit p: Parameters) extends PhrModule with HasPhrParameters with Helpers {
  ...
  redirectData.phrMeta := io.train.redirect.bits.meta.phrDiff
  ...
  private val predictHist   = getRedirectPhr(bpTrain.meta.phrDiff)
