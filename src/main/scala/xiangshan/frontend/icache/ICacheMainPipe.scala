...
class ICacheMainPipe(implicit p: Parameters) extends ICacheModule
  ...
  class ICacheMainPipeIO(implicit p: Parameters) extends ICacheBundle {
    ...
    val wayLookupRead:   DecoupledIO[Vec[WayLookupBundle]] = Flipped(DecoupledIO(Vec(2, new WayLookupBundle)))
    val secondReadValid: Bool                              = Output(Bool())
    ...
    /* *** outside interface *** */
    // Ftq
    val fromFtq:      DecoupledIO[FtqToMainPipeBundle] = Flipped(Decoupled(new FtqToMainPipeBundle))
  ...
  /* *** Input/Output port *** */
  private val (fromFtq, toIfu)   = (io.fromFtq, io.resp)
  ...
  private val s0_req   = fromFtq.bits.req

  private val s0_ftqIdx = s0_req(0).ftqIdx

  private val s0_twoFetchValid = s0_req(1).valid

  private val s0_wayLookupEntry = VecInit(fromWayLookup.bits.map(_.entry))
  private val s0_exceptionInfo  = VecInit(fromWayLookup.bits.map(_.exceptionEntry))

  private val s0_wayMask = VecInit(s0_wayLookupEntry.map(_.waymask))

  io.secondReadValid := false.B // FIXME
  ...
  toData.bits.zipWithIndex.foreach { case (readReq, i) =>
    readReq.valid        := (if (i == 0) true.B else s0_twoFetchValid)
  ...
  private val s1_twoFetchValid  = RegEnable(s0_twoFetchValid, s0_fire)
  ...
  private val s1_maybeRvcMap = VecInit(
    VecInit((0 until DataBanks).map { i =>
      DataHoldBypass(
        Mux(
  ...
  toIfu.valid                   := s1_fire
  toIfu.bits.doubleline         := s1_req(0).isCrossLine
  toIfu.bits.data               := s1_datas.asUInt
  toIfu.bits.maybeRvcMap        := s1_maybeRvcMap.asUInt
  toIfu.bits.isBackendException := s1_req(0).isBackendException
  toIfu.bits.vAddr              := VecInit(s1_vAddr(0), s1_vAddr(1))
  toIfu.bits.pAddr              := getPAddrFromPTag(s1_vAddr.head, s1_pTag)
  toIfu.bits.exception          := s1_exceptionOut
  toIfu.bits.pmpMmio            := s1_pmpMmio
  toIfu.bits.itlbPbmt           := s1_wayLookupEntry(0).itlbPbmt
  // valid only for the first gpf
  toIfu.bits.gpAddr            := s1_exceptionInfo(0).gpAddr
  toIfu.bits.isForVSnonLeafPTE := s1_exceptionInfo(0).isForVSnonLeafPTE
