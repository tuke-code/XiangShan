...
//  XSError(ifuWbPtr < commitPtr && !isFull(ifuWbPtr(0), commitPtr(0)), "ifuWbPtr runs ahead of commitPtr")
  ...
  when(io.toICache.toMainPipe.fire) {
    val twoFetchValid = io.toICache.toMainPipe.bits.req(1).valid
    ifuPtr := Mux(twoFetchValid, ifuPtr + 2.U, ifuPtr + 1.U)
  }
  ...
  private val twoPrefetchCase = TwoPrefetchCase(prefetchReq(0), prefetchReq(1))

  private val twoPrefetchValid = twoPrefetchCase.valid && distanceBetween(bpuPtr(0), pfPtr(0)) >= 3.U &&
...
//  private val twoPrefetchValid = false.B
  ...
  private val fetchReq = VecInit(
    Wire(new FtqFetchReq).fromFtqEntry(entryQueue(ifuPtr(0).value), twoFetchInfoVec(ifuPtr(0).value)),
    Wire(new FtqFetchReq).fromFtqEntry(entryQueue(ifuPtr(1).value), twoFetchInfoVec(ifuPtr(1).value))
  )

  // FIXME: when no hit, waymask=0?
  private val isICacheDataSramReadConflict =
    (0 until DataBanks).map { bankIdx =>
      val (firstReqReadValid, firstReqReadSetIdx, firstReqReadWayMask) = {
        val lineSelOH     = Cat(fetchReq(0).bankSel(1)(bankIdx), fetchReq(0).bankSel(0)(bankIdx))
        val readBankValid = lineSelOH.orR
        val readSetIdx    = Mux1H(lineSelOH, fetchReq(0).vSetIdx)
        val readWayMask   = Mux1H(lineSelOH, fetchReq(0).wayMask)
        (readBankValid, readSetIdx, readWayMask)
      }
      val (secondReqReadValid, secondReqReadSetIdx, secondReqReadWayMask) = {
        val lineSelOH     = Cat(fetchReq(1).bankSel(1)(bankIdx), fetchReq(1).bankSel(0)(bankIdx))
        val readBankValid = lineSelOH.orR
        val readSetIdx    = Mux1H(lineSelOH, fetchReq(1).vSetIdx)
        val readWayMask   = Mux1H(lineSelOH, fetchReq(1).wayMask)
        (readBankValid, readSetIdx, readWayMask)
      }

      // single-port SRAM can read only one set
      firstReqReadValid && secondReqReadValid &&
      firstReqReadWayMask === secondReqReadWayMask &&
      firstReqReadSetIdx =/= secondReqReadSetIdx
    }.reduce(_ || _)

//  private val twoFetchValid =
//    (fetchReq(0).size +& fetchReq(1).size) <= FetchBlockSize.U &&
//      !isICacheDataSramReadConflict &&
//      fetchReq(0).vPageNumber === fetchReq(1).vPageNumber && // only one pmp
//      !fetchReq(0).isMmio && !fetchReq(1).isMmio &&
//      !(backendException.hasException && (
//        backendExceptionPtr === ifuPtr(0) || backendExceptionPtr === ifuPtr(1)
//      ))

  private val twoFetchValid = false.B

  private val ifuReqValid = bpuPtr(0) > ifuPtr(0) && !redirect.valid &&
    distanceBetween(ifuPtr(0), commitPtr(0)) < (FtqSize - 1).U

  io.toICache.toMainPipe.valid := ifuReqValid
  io.toICache.toMainPipe.bits.req.zipWithIndex.foreach { case (req, i) =>
    req.valid              := (if (i == 0) true.B else twoFetchValid)
    ...
    req.isBackendException := backendException.hasException && backendExceptionPtr === ifuPtr(i)
  }
