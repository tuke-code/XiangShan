...
class ResolveQueue(implicit p: Parameters) extends FtqModule with HalfAlignHelper with HasCircularQueuePtrHelper {
  ...
  XSPerfAccumulate(
    ...
    io.backendResolve.map(branch => branch.valid && branch.bits.attribute.isDirect).reduce(_ || _)
  )
  XSPerfAccumulate(
    "backend_return_resolve",
    io.backendResolve.map(branch => branch.valid && branch.bits.attribute.isReturn).reduce(_ || _)
  )
