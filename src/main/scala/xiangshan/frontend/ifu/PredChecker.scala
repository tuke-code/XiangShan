...
class PredChecker(implicit p: Parameters) extends IfuModule {
  ...
  io.resp.stage1Out.fixedTaken := pdInfoVec.zipWithIndex.map { case (pd, i) =>
    pd.brAttribute.isIndirect || pd.brAttribute.isDirect || (isPredTaken(i) && !pd.notCFI)
  }

  private val firstBlockFixedTakenInstrIdx =
    WireDefault(0.U.asTypeOf(ValidUndirectioned(UInt(FetchBlockInstOffsetWidth.W))))
  firstBlockFixedTakenInstrIdx.valid := ParallelOR(firstBlockFixedTaken)
  firstBlockFixedTakenInstrIdx.bits  := ParallelPriorityEncoder(firstBlockFixedTaken)

  private val secondBlockFixedTakenInstrIdx =
    WireDefault(0.U.asTypeOf(ValidUndirectioned(UInt(FetchBlockInstOffsetWidth.W))))
  secondBlockFixedTakenInstrIdx.valid := ParallelOR(secondBlockFixedTaken)
  secondBlockFixedTakenInstrIdx.bits  := ParallelPriorityEncoder(secondBlockFixedTaken)
  ...
  private val seqTargets = VecInit((0 until IBufferEnqueueWidth).map(i =>
    instrPcVec(i) + Mux(pdInfoVec(i).isRVC || (invalidTaken(i) && !notCfiTaken(i)), 2.U, 4.U)
  ))
  ...
  private val fixedIsJump = instrValid(mispredIdx.bits) && mispredIdx.valid &&
  ...
  private val finalNotCfiTakenNext = RegEnable(finalNotCfiTaken, io.req.valid)
  private val finalSelectBlockNext = RegEnable(finalSelectBlock, io.req.valid)
  private val finalFirstTakenNext  = RegEnable(firstBlockFixedTakenInstrIdx.valid, io.req.valid)
  private val finalSecondTakenNext = RegEnable(secondBlockFixedTakenInstrIdx.valid, io.req.valid)
  ...
  private val fixedTaken = !invalidTakenNext && Mux(finalSelectBlockNext, finalSecondTakenNext, finalFirstTakenNext)
  ...
  io.resp.stage2Out.checkerRedirect.bits.notCfiTaken  := finalNotCfiTakenNext
