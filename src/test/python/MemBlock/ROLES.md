...
- `CHANGELOG.md` 要按日期维护统一格式：
  - 不同日期按日期倒序排列，最近日期在前。
  - 同一日期只保留一个 `## YYYY-MM-DD` 日期标题。
  - 同一日期下的多次修改按从上到下顺序排列，使用 `### 1. ...`、`### 2. ...` 这类编号标题。
  - 如果某条修改内部已有标题层级，则在并入同日编号条目后整体增加一级，例如条目标题下的 `变更摘要` 使用 `####`。
  - 允许新增内容，但不要重写既有条目的事实表述或把旧条目悄悄改成另一套口径。
...
- 公共接口问题先收敛，再推动并行开发。
- 维护 `CHANGELOG.md` 时优先保证日期倒序、同日单标题、同日编号条目和标题层级统一，而不是只做尾部追加。
...
- `CHANGELOG.md` 是否满足日期倒序、同日单日期标题、同日 `### 1. ...` 编号条目，以及条目内部标题随之升一级的格式约束。
