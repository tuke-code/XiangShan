...
- 因此当前缺口已经从“权限类 fault 场景明显不足”收敛成“load-side baseline 已有，但 store-side 与更深的 allow/deny 动态窗口仍缺”

#### 建议新增用例

1. store deny
2. allow/deny 切换
3. 同一地址空间下的 load/store 混合 deny
4. 更长窗口下的 fault -> replay/writeback/exceptionVec 一致性
