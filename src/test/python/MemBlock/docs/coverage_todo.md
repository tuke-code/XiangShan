...
本轮验证还额外暴露出一个实现事实：如果先产生 MMIO outer write，再直接走 `flush_store_buffers_and_wait()` 的最终 drain compare，当前 scoreboard 会把该 outer write 计入 `drain_log`，从而与“MMIO 不进入最终 golden compare”的目标相冲突。换句话说，这里不仅缺 testcase，也可能存在环境语义缺口。
