...
`_handle_dcache_a()` 当前支持两类请求：

1. `TL_A_ACQUIRE_BLOCK`
   - 按 cacheline 对齐请求地址。
   - 从 `self.memory` 读取一整行数据。
   - 切成两个 32B beat。
   - 若 `echo_isKeyword=1`，按 RTL 期望翻转 beat 顺序。
   - 为本次 grant 分配唯一 `sink`。
   - 延迟若干拍后在 D 通道返回 `TL_D_GRANT_DATA` beats。

2. `TL_A_CBO_CLEAN` / `TL_A_CBO_FLUSH` / `TL_A_CBO_INVAL`
   - 记录请求地址、source 与 `echo_isKeyword` 供调试统计使用。
   - 不从 `self.memory` 读取 cacheline，也不伪造任何 drain 写回。
   - 延迟若干拍后在 D 通道返回单拍 `TL_D_CBO_ACK`。
   - 不分配 grant `sink`，也不进入 E 通道闭环。
...
这保证了 dcache `AcquireBlock -> GrantData* -> GrantAck` 闭环的完整性。
对 non-zero CBO 来说，当前模型返回的是 `CBOAck`，不要求后续 `GrantAck`。
...
1. dcache A 通道目前只支持 `AcquireBlock` 与 non-zero CBO；`AcquirePerm` 等其它 opcode 仍未建模。
2. sbuffer `wline` drain 目前只对 `cbo.zero` 建模整行 memory effect；`cbo.clean/flush/inval` 只校验 dcache `A -> D(CBOAck)` 收口，不伪造 drain 数据。
